...
export async function AboutMissionValues() {
  const locale = (await getLocale()) as Locale;
  const t = await getTranslations("about");
  const missionDescription = getAboutMissionDescription(locale);
  const values = getAboutValues(locale);
  const whyChooseUs = getAboutWhyChooseUs(locale);
      ...
      <div className={`${aboutMissionGridClass} items-stretch gap-0`}>
        ...
        <div className={`${aboutAustraliaImageBoxClass} bg-[#002147]`}>
          <Image
            ...
            aria-hidden="true"
          />
          <div className="absolute inset-0 bg-[#002147]/35" aria-hidden="true" />
